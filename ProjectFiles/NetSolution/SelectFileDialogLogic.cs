...
public class SelectFileDialogLogic : BaseNetLogic
{
	public override void Start()
	{
		...
		filterVariable = widgetBackground.GetVariable("ExtensionFilter");
		...
		showHiddenFilesVariable = widgetBackground.GetVariable("ShowHiddenFiles");
		...
		{
			Log.Error(System.Reflection.MethodBase.GetCurrentMethod().Name, $"Path variable '{pathVariable.Value.Value}' is invalid. Falling back to '%PROJECTDIR%\\'");
	...
	private void Browse(ResourceUri resourceUri)
	{
		...
		{
			Log.Warning(System.Reflection.MethodBase.GetCurrentMethod().Name, "Path variable is empty");
			return;
		}

		if (!Directory.Exists(path))
		{
			Log.Warning(System.Reflection.MethodBase.GetCurrentMethod().Name, $"Path '{path}' does not exist");
			return;
		}
		...
		{
			var backEntry = InformationModel.MakeObject<SelectFileDialogEntry>("back");
			backEntry.FileName = "..";
	...
	private SelectFileDialogEntry CreateFilesystemEntry(FileSystemInfo entry, bool isDirectory)
	{
		var fileSystemEntry = InformationModel.MakeObject<SelectFileDialogEntry>(entry.Name);
...
namespace FilesystemBrowserHelper
{
	...
	static class PlatformConfigurationHelper
	{
		...
		public static bool IsWindowsNetworkDriveAccessible(string filePath)
		{
			...
			{
				Log.Error(System.Reflection.MethodBase.GetCurrentMethod().Name, $"Unable to determine connected network drives: {exception.Message}");
				return false;
			}
